...
import { getTransactionDepositRequests } from '../../../actions/transactions/index';
...
const Deposit = (props) => {
  ...
  useEffect(() => {
    document.title = "BtcEX | Deposits";
  }, []);

  // console.log("DEPOSIT", auth);s

  useEffect(() => {
    dispatch(getTransactionDepositRequests(auth.userId, offset));
  ...
  return (
    <>
      ...
      <Sidebar />
      <div className="main">
        <TransactionNavbar active="/transactions/deposit" />
          ...
          <div className="row justify-content-center">
            <div className="col-lg-12">
              <Table responsive="lg" striped hover>
                ...
                <tbody>
                  {
                    transactions.deposits.map((d, index) => {
                      let info = d.transactionInfo;
                      let value = null;
                      if (info) {
                        let txHash = info.link || info.url || info.transactionHash || info.txHash;
                        if (txHash) {
                          const explorerPrefix = AppConstants.EXPLORE_MAP[d.currency];
                      ...
                      return (
                        <tr key={index}>
                          <td>{dayjs(d.createdAt).format('MMM D, YYYY h:mm A')}</td>
                          <td>{d.status}</td>
                          <td>{d.amount}</td>
                          <td>{d.currency}</td>
                          <td>{d.refNo || "-"}</td>
